...
Location(can be run in Notebooks):
	"\Graph\Assignment_induced_datum\1_Assignment_induced_datum.ipynb"
	
	
Name:
	2_phi_calculator.ipynb
Description:
	We implement the definition of a phi-stability condition, which allows one to generate any phi-stability condition given a graph and a non-degenerate phi.
Location:
	\Stability_Conditions\Graph\phi_investigation\2_phi_calculator.ipynb
...
Location(can be run in Notebooks):
	"\Graph\genus_1_graphs\3_Cycles_to_stability.ipynb"
...
Location:
	\Stability_Conditions\Graph\phi_investigation\6_phi_analysis.ipynb
...
Location:
\Stability_Conditions\Graph\phi_investigation\examples\FV_G6M3M14\7_strong_stability_check.ipynb


Name:
	8_counterexample.ipynb
Description:
	We investigate a single example of a stability condition not induced by a polarisation, and produce the Fillipo Viviani function related to it.
Location:
	\Stability_Conditions\Graph\phi_investigation\examples\FV_G6M3M14\8_counterexample.ipynb


Name:
	9_find_cycles.ipynb
Description: Given set of stability conditions where each is stored in the form (A_{\Gamma},\sigma_{\Gamma}^{A_{\Gamma}}(\Gamma)) we find the appropriate cycle for each genus 1 connected spanning subgraph.
Location:
	"\Stability_Conditions\Graph\find_cycles\find_cycles.ipynb"


Name:
	10_cycle_analysis.ipynb
Description:We decompose the sets of stability conditions up to translations (for each graph), by fixing stability conditions on a fixed subgraph with a maximal necklace, and investigate
bounds for the number of stabiity conditions.
Location:	
	"C:\Stability_Conditions\Graph\find_cycles\10_cycle_analysis.ipynb"
